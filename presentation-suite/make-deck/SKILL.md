...
- **No context** — Claude picks an aesthetic direction from scratch (named reference style, character fonts, restrained palette) and states it in one paragraph before building
- **Decide for me** — same: Claude commits to a direction and vocalizes it
...
- Palette (pull from loaded tokens, or compose one yourself in `oklch()` harmony)
